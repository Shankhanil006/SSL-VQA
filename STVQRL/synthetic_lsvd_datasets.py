...
class LSVDDataset(Dataset):
    def __init__(self, lsvd_dir, ps = 224, depth = 32,  k = 9, fps = 2, transform=transforms.ToTensor()):
        ...
        self.num_clips = 2
        self.sampler =  SampleFrames(clip_len = self.depth,frame_interval = 2, num_clips=self.num_clips)
    ...
    def __getitem__(self, idx):
        ...
        sampled_clip = get_spatial_fragments(all_clips, aligned = self.depth*2)
        # clip_view1 =  sampled_clip
        # clip_view2 =  sampled_clip[:,:, :-1:self.fps, ...]
        tmp = torch.chunk(sampled_clip, chunks=2, dim = -3)
        clip_view1 = tmp[0]
        clip_view2 = tmp[1]
