...
video = decord.VideoReader('path to video')           
...
reg_score = pred.mean().item()
niqe_score = np.exp(-distance(pris_feat, dist_feat)*tau).mean()
vid_score = (reg_score + niqe_score)/2

duration = (time.time() - start)/60.0
print(vid_score, reg_score, niqe_score,  duration)
